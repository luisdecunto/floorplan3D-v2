...
The production collaboration endpoint is `https://planform-collaboration.luisdcnt.workers.dev`, hosted in the `luisdcnt@gmail.com` Cloudflare account. Set `VITE_COLLABORATION_URL` to override it for another deployment. The legacy Cocoscraper endpoint remains online only for old invite links; Durable Object room state is account-bound and cannot be moved between accounts.
